...
.controller('remoteCtrl', function($scope, $stateParams, $dbHelper) {
  ...
  $scope.makeconnect = function(){
    chrome.sockets.tcp.create({}, function(createInfo) {
      console.dir($scope.remoteClient);
      sockid=createInfo.socketId;
      chrome.sockets.tcp.connect(createInfo.sockId,$scope.remoteClient.ip.toString(), parseInt($scope.remoteClient.port),function(){
        $scope.flag = "success";
      });
    });
  }
  ...
  function connect(data) {
    ...
    function() {
      ...
      */
      chrome.sockets.tcp.create({},function(createInfo){
        sockid = createInfo.sockId;
        notConnected = false;
        console.dir(createInfo);
        console.log($scope.remoteClient.ip.toString());
        console.log(parseInt($scope.remoteClient.port));
        chrome.sockets.tcp.connect(createInfo.sockId,$scope.remoteClient.ip.toString(), parseInt($scope.remoteClient.port),connectSucs);
      });
    },function(error) {
